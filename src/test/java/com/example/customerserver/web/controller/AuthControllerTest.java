...
class AuthControllerTest {
	...
	void setUp() {
		...
		mockMvc = MockMvcBuilders.standaloneSetup(authController, exceptionController)
			.setCustomArgumentResolvers(
				new CodeArgumentResolver(codeGenerator),
				new AccessTokenArgumentResolver(accessTokenGenerator, objectMapper))
	...
	@WithMockCustomCustomer
	void redirectWithCodeTest() throws Exception {
	...
	void isUnauthorizedRequest() throws Exception {
		mockMvc.perform(get("/auth/code")
				.cookie(new Cookie("redirectUrl", redirectUrl)))
	...
	@WithMockCustomCustomer
	@Transactional
	void validCodeAnd() throws Exception {
		//given
		final Customer customer = saveCustomer();
		final String code = codeGenerator.createToken(String.valueOf(customer.getId()));
		final String clientId = clientSteps.clientRegisterWithDefaultStep();
	...
	@Transactional
	void CodeValidFail() throws Exception {
		//given
		final Customer customer = saveCustomer();
		final String code = codeGenerator.createToken(String.valueOf(customer.getId())) + "noisy";
		final String clientId = clientSteps.clientRegisterWithDefaultStep();
